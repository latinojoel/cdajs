...
(function(window) {
    ...
    if (window.XMLHttpRequest) {
      ...
      (xhr = function() {
        ...
        open: function(method, url, async, username, password) {
          if (async !== true) { throw "Synchronous mode not supported in this environment."; }
        ...
        send: function(data) {
          var me = this, options = me.options, client;
          if (data) {
  ...
  CDA.prototype.doQuery = function(callback, options) {
    ...
    if (options.params.dataAccessId === undefined || options.params.dataAccessId == null) {
      CDA.Exception.newError("ERROR_MISSING_DATA_ACCESS_ID", "doQuery", null)._throw();
    }
    ...
    options.params["userid"] = options.username || this.options.username;
    options.params["password"] = options.password || this.options.password;

    options.form = _convertParams(options.params);
    options.url = (options.url || this.options.url) + "doQuery" + (options.method.toUpperCase() === "GET" ? (options.form || "") : "");
  ...
  CDA.Exception.newError = function(codeName, source, data) {
    return new this.newDetailError(CDA.Exception[codeName + "_CODE"], CDA.Exception[codeName + "_MSG"], CDA.Exception[codeName + "_HELP"], source,
        data);
  };
  ...
  CDA.Exception.ERROR_CDA_OPTIONS_HELP = "Please provide CDA options on the CDA object constructor (Please consult this page "
      + CDAWikiExceptionCodes + ")";
  ...
  CDA.Exception.ERROR_CDA_USERNAME_HELP = "Please provide CDA username on the CDA object constructor (Please consult this page "
      + CDAWikiExceptionCodes + ")";
  ...
  CDA.Exception.ERROR_CDA_PASSWORD_HELP = "Please provide CDA password on the CDA object constructor (Please consult this page "
      + CDAWikiExceptionCodes + ")";
